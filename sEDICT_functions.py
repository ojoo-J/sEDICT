...
def sEDICT_editing(im_path,
    ...
    """
    # d step
    # if d_steps is None: d_steps = steps
    ...
    # add noise
    if noise_seed is not None: seeds = np.arange(noise_seed, noise_seed+n_sample) # for reproduce
    else: seeds = [random.randrange(2**32 - 1) for _ in range(n_sample)]
    
    if noise_scale is not None:
        gens = []
        for j in range(n_sample):
            new_latents = [None, None]
            #torch.cuda.manual_seed(seeds[j])
            random.seed(seeds[j])
            torch.manual_seed(seeds[j])
            torch.cuda.manual_seed_all(seeds[j])
            noise = noise_scale * torch.randn_like(latents[0])
            new_latents[0] = latents[0].detach().clone() + noise
            if not run_baseline: new_latents[1] = latents[1].detach().clone() + noise
            ...
            gen = coupled_stablediffusion(edit_prompt if (not use_p2p) else base_prompt,
                                        ...
                                        fixed_starting_latent=new_latents,
                                         init_image_strength=d_init_image_strength,
                                        steps=steps,
                                        mix_weight=mix_weight,
                                         guidance_scale=guidance_scale,
                                         run_baseline=run_baseline,
                                        )
            gen[0].save(save_dir + f'{base_prompt}_{noise_scale}_{steps}_{init_image_strength}_{d_init_image_strength}_{seeds[j]}.png')
